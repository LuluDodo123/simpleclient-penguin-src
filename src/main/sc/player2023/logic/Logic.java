...
public class Logic implements IGameHandler {
  ...
  private static final Debug debug = new Debug(Debug.ANZAHL_FELDER, Debug.ANZAHL_FISCHE, Debug.STOP_GRUND, Debug.SCANNED_FIELD, Debug.MOVE_POINTS);
  ...
  public Move movePinguin() {
    ...
    for (Move move: gameState.getPossibleMoves()) {
      ...
      foreachReachableFields(move, (field, direction) -> {
        options[3 - field.getFish()]++;
      });
      // Todo: balancing
      points += 33 * move.getTo().getFish();
      points += (int) Math.round(Math.sqrt(90 * options[0]));  
      points += (int) Math.round(Math.sqrt(30 * options[1]));
      points += (int) Math.round(Math.sqrt(10 * options[2]));

      Debug.setArg(Debug.MOVE_POINTS, move.getTo(), points, maxPoints);
      Debug.log(Debug.MOVE_POINTS);
